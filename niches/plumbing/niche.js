...
function renderContent(c){
  ...
  if(pg) pg.innerHTML = (c.pricing||[]).map(function(p){
    var feat = p.highlight ? ' feat' : '';
    var pill = p.highlight ? '<span class="pill">Most common</span>' : '';
    var items = (p.features||[]).map(function(f){ return '<li><svg viewBox="0 0 24 24"><path d="M4 12l6 6L20 6"/></svg>'+esc(f)+'</li>'; }).join('');
    return '<div class="pcard'+feat+'">'+pill+'<div class="lbl">'+esc(p.label)+'</div><div class="price">'+esc(p.blurb)+'</div><span class="per">'+esc(p.per)+'</span><ul>'+items+'</ul></div>';
  }).join('');
