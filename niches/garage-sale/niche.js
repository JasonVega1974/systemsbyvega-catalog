/* garage-sale/niche.js — this niche's renderer and interactive logic.
   ...
   Content shape notes: hero/about/howItWorks/prepTips/contact live under
   content.json's `niche` namespace and reach this renderer FLAT (base.js
   SLflat). brand carries a SPLIT city + state — the estate-sale convention,
   shared only by these two siblings — composed here wherever a locale line
   renders. pricing is the percentage-model OBJECT {commission, minimum, note}
   (spec Decision 3), never an array. niche.contact {heading, body, hours} is
   this niche's new key family; the flattened c.contact.hours doubles as the
   footer-contact component's hours line for free. */
(function () {
  'use strict';
  ...
  function smsBody(){ return 'Hi ' + brandName() + "! I'd like to book a free driveway walkthrough."; }
  ...
  /* The percentage sale-sticker card. commission + minimum are the admin's
     two editable fields for this model; note rides along on the same object.
  ...
  /* Six prep-tip cards, order preserved from content. Every value lands via
     textContent; the numbered sticker dot is CSS-counter chrome. */
  function renderPrepTips(tips){
    var g = document.getElementById('tipsGrid');
    if (!g || !tips || !tips.length) return;
    while (g.firstChild) g.removeChild(g.firstChild);
    tips.forEach(function (t, i) {
      var tip = document.createElement('div');
      tip.className = 'tip ' + revealCls();
      tip.setAttribute('data-delay', String(i % 3 + 1));
      var dot = document.createElement('span'); dot.className = 'tip__dot';
      var h3 = document.createElement('h3'); h3.textContent = t.title || '';
      var p = document.createElement('p'); p.textContent = t.desc || '';
      tip.appendChild(dot); tip.appendChild(h3); tip.appendChild(p);
      g.appendChild(tip);
    });
  }

  /* niche.contact {heading, body, hours} — the booking section's voice. */
  function renderContact(ct){
    if (!ct) return;
    setText('contactHeading', ct.heading);
    setText('contactBody', ct.body);
    setText('contactHours', ct.hours);
  }
  ...
  function renderContent(c){
    var b = c.brand || {};
    /* Full name everywhere; the nav lockup drops a trailing " Garage Sales" —
       the line under it already says "Garage sales · <city, state>". */
    ...
    Array.prototype.forEach.call(document.querySelectorAll('[data-brand-short]'), function (el) {
      el.textContent = String(b.name || '').replace(/\s+Garage Sales$/i, '');
    });
    ...
    if (bl && locale) bl.textContent = 'Garage sales · ' + locale;
  ...
  if (form) {
    ...
    var buildSms = function (d) {
      var body = 'Hi ' + brandName() + "! I'd like to book a free driveway walkthrough."
        + ' Name: ' + d.name + '.'
        + ' Phone: ' + d.phone + '.'
        + ' Sale address: ' + d.address + (d.city ? ', ' + d.city : '') + '.'
        + (d.date ? ' Preferred weekend: ' + d.date + '.' : '')
        + (d.notes ? ' In the garage: ' + d.notes + '.' : '');
      ...
      document.addEventListener('click', function (e) {
        ...
        if (!t) {
          ...
          t.style.cssText = 'position:fixed;left:50%;bottom:32px;transform:translateX(-50%);z-index:9999;'
            + 'background:#FFFFFF;color:#33241A;padding:14px 22px;border-radius:14px;'
            + 'box-shadow:0 12px 40px rgba(140,75,10,.35);border:1.5px solid rgba(221,119,8,.5);'
            + 'font-family:\'Public Sans\',system-ui,sans-serif;font-size:15px;max-width:88vw;text-align:center;'
            + 'opacity:0;transition:opacity .25s ease;';
          document.body.appendChild(t);
        }
        t.innerHTML = 'Call or text us at <strong style="color:#A8500A;letter-spacing:.02em">' + esc(brandPhone()) + '</strong> from your phone.';
